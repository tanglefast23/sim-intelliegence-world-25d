...
- Build the chair as separate seat, back, and armrest parts. For a visible face, keep the seat and
  back behind the body, then place only the camera-side armrest above the solid body. Only an arm
  may cross above that armrest. An approved loose sheet or drape may cover it.
- Lower every authored sitter from the standing contact anchor by the same small shared offset.
  Judge the result in pixels at shipping zoom; do not move each character by eye.
