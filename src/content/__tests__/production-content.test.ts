...
describe('the production cast repair', () => {
  ...
  test('removes retired clerks and restores every approved worker to their seat', () => {
    const initial = createInitialState();
    const legacyNpc = { ...initial.npcs.clerk_01!, id: 'clerk_09' };
    const legacySchedule = { ...initial.schedules.clerk_01_daily!, id: 'clerk_09_daily', npcId: 'clerk_09' };
    const legacy = {
      ...initial,
      maps: Object.fromEntries(Object.entries(initial.maps).map(([id, map]) => [
        id, { ...map, active: id === 'west_office' },
      ])),
      npcs: {
        ...initial.npcs,
        clerk_01: {
          ...initial.npcs.clerk_01!,
          presence: { kind: 'active_local' as const, mapId: 'west_office', locationId: 'ledger_annex', tileX: 10, tileY: 10 },
        },
        clerk_09: legacyNpc,
      },
      schedules: { ...initial.schedules, clerk_09_daily: legacySchedule },
    };

    const repaired = migrateProductionSchedules(legacy);
    expect(repaired.npcs.clerk_09).toBeUndefined();
    expect(repaired.schedules.clerk_09_daily).toBeUndefined();
    for (const staff of PRODUCTION_OFFICE_STAFF) {
      expect(repaired.npcs[staff.id]!.presence).toEqual({
        kind: 'active_local', mapId: 'west_office', locationId: 'ledger_annex',
        tileX: staff.work.x, tileY: staff.work.y,
      });
    }
  });
});
