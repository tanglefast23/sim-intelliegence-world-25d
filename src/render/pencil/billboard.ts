...
/** Pull a front-facing sitter toward the camera without moving them away from the chair on screen. */
...
export function pencilBillboards(frame: WorldFrameState): readonly BillboardDescriptor[] {
  return frame.characters
    ...
    .map((character) => {
      ...
      lift: seated && !hasAuthoredSeat ? SEATED_LIFT_TILES : -(CONTACT_SINK_WORLD_PIXELS * scale) / TILE_SIZE,
      depthBias: hasAuthoredSeat && !chairOccludesBody ? AUTHORED_FRONT_SEAT_LAYER_TILES : 0,
      };
    });
}
