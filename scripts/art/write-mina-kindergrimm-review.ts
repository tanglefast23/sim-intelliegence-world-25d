...
import {
  ...
  MINA_PART_IDS,
  renderGeneratedMinaFrame,
  renderMinaWireframe,
} from '../../src/render/pencil/generated-mina';
import { type VampirePose } from '../../src/render/pencil/pose';
import { buildPencilLayout } from '../../src/render/pencil/layout';
import { drawClassicWitch } from '../../src/render/pencil/parts/witch';
import { VAMPIRE_FACINGS } from '../../src/render/pencil/pose';
...
function bakeCurrentMinaFrames(): readonly Uint8ClampedArray[] {
  const recipe = PENCIL_CHARACTER_RECIPES['mina-park'];
  ...
  return VAMPIRE_FACINGS.flatMap((facing) => [false, true].flatMap((moving) => (
    (moving ? [0, 1] : [0]).flatMap((gait) => Array.from({ length: BOIL_FRAMES }, (_, boil) => {
      ...
      drawClassicWitch(sketch, layout, { facing, moving, gait: gait as 0 | 1 });
      return sketch.data;
    }))
  )));
}
...
function writeReview(scale: number, filename: string): string {
  const current = bakeCurrentMinaFrames();
  const generated = bakeGeneratedMinaFrames(true);
  const rowHeight = PENCIL_HEIGHT * scale + LABEL_HEIGHT;
  ...
  paintRow(output, scale, 2, 'RIG DATA / BLUE JOINT RED HAND GREEN CONTACT GOLD FACE', FACINGS.map((facing) => renderMinaWireframe(pose(facing, 0, false), true)));
  paintRow(output, scale, 3, 'WALK GAIT 0 / FRONT REAR LEFT RIGHT', FACINGS.map((facing) => renderGeneratedMinaFrame(pose(facing, 0, true), 0, true)));
  paintRow(output, scale, 4, 'WALK GAIT 1 / FRONT REAR LEFT RIGHT', FACINGS.map((facing) => renderGeneratedMinaFrame(pose(facing, 1, true), 0, true)));
  paintRow(output, scale, 5, `SILHOUETTE / ${MINA_PART_IDS.length} ORDERED PARTS`, IDLE_INDEXES.map((index) => generated[index]!), PAPER, true);

  const directory = resolve('artifacts/kindergrimm-characters/mina');
...
process.stdout.write(`${writeReview(1, 'mina-review-1x.png')}\n`);
process.stdout.write(`${writeReview(3, 'mina-review-3x.png')}\n`);
const current = bakeCurrentMinaFrames();
const updated = bakeGeneratedMinaFrames(true);
process.stdout.write(`${writeFacingSheet('mina-current-3x.png', 'CURRENT / FRONT REAR LEFT RIGHT', IDLE_INDEXES.map((index) => current[index]!))}\n`);
process.stdout.write(`${writeFacingSheet('mina-kindergrimm-3x.png', 'KINDERGRIMM / FRONT REAR LEFT RIGHT', IDLE_INDEXES.map((index) => updated[index]!))}\n`);
process.stdout.write(`${writeFacingSheet('mina-rig-data-3x.png', 'RIG DATA / FRONT REAR LEFT RIGHT', FACINGS.map((facing) => renderMinaWireframe(pose(facing, 0, false), true)))}\n`);
